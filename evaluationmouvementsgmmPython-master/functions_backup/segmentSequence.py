    ...
    for t in range(variation.shape[1]):
        if kp == 1:
            ...
            if variation[0][t] < thres:
                mintab = np.hstack((mintab, t + 1))
                kp = 1
    cuts = mintab.astype(int) # in Python, indice begins with 0 while in Matla  b with 1
    return cuts, variation


def segmentSequence(_in, ws, thres):  ##return 1-d vector and matrix
    variation = np.full((1, _in.shape[1]), np.nan)
    for t in range(math.ceil(ws / 2), _in.shape[1] - math.floor(ws / 2) + 1):
        sigma = 0
        w = math.floor(ws / 2)
    ...
    if mintab.shape[1] > 0:
        cuts = mintab[:, 0]  # in Python, indice begins with 0 while in Matlab with 1
    else:
        cuts = np.array([])
